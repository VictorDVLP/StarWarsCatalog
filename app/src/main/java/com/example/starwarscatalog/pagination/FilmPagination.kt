...
class FilmPagination (private val service: StarWarsApi):
    PagingSource<Int, FilmDb>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, FilmDb> {
        val position = params.key ?: START_INDEX
        return try {
            val response = service.retrofitService.getItemFilms(position)
    ...
    override fun getRefreshKey(state: PagingState<Int, FilmDb>): Int? {
        return null
    }
}