...
import com.example.starwarscatalog.adapters.SpecieAdapter
import com.example.starwarscatalog.databinding.FragmentSpeciesBinding
...
class SpeciesFragment: Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = SpecieAdapter {
            findNavController().navigate(
                R.id.action_speciesFragment_to_specieDetailFragment, bundleOf("specie" to it)
            )
        }
        lifecycleScope.launch {
            viewModel.responseSpecies.collectLatest { pagingData ->
                adapter.submitData(
                    pagingData
                )
            }
        }
        binding.recyclerSpecies.layoutManager = GridLayoutManager(requireContext(), 2)
