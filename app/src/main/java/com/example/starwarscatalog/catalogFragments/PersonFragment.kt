...
import com.example.starwarscatalog.adapters.PeopleAdapter
import com.example.starwarscatalog.databinding.FragmentPersonBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = PeopleAdapter {
            findNavController().navigate(
                R.id.action_personFragment_to_personDetailFragment, bundleOf("people" to it)
            )
        }
        lifecycleScope.launch {
            viewModel.responsePeople.collectLatest { pagingData ->
                adapter.submitData(
                    pagingData
                )
            }
        }
        binding.recyclerPeople.layoutManager = GridLayoutManager(requireContext(), 2)
